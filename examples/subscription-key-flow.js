...
const createSubscriptionWithKey = async () => {
  try {
    ...
    const { success, subscriptionId } = await sdk.createSubscription(subscriptionParams);
    ...
    const contractId = process.env.NEXT_PUBLIC_contractId || 'subscription.testnet';
    ...
    const registerResult = await sdk.registerSubscriptionKey(subscriptionId, keyPair.publicKey);

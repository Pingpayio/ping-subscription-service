...
import { JobSchema, JobInput, Job, JobData, ScheduleType, JobStatus } from '../types/job.js';
import { calculateInitialDelay, calculateRepeatOptions, calculateNextRun, formatDate } from '../lib/scheduler.js';
...
app.post('/jobs', async (c) => {
  try {
    ...
    if (!result.success) {
      return c.json({ message: 'Invalid job data', errors: result.error.format() }, 400);
    }
    ...
    return c.json({ message: 'Error creating job', error: (error as Error).message }, 500);
  }
});
...
app.get('/jobs', async (c) => {
  try {
    ...
    return c.json({ message: 'Error getting jobs', error: (error as Error).message }, 500);
  }
});
...
app.get('/jobs/:id', async (c) => {
  try {
    ...
    return c.json({ message: 'Error getting job', error: (error as Error).message }, 500);
  }
});
...
app.put('/jobs/:id', async (c) => {
  try {
    ...
    return c.json({ message: 'Error updating job', error: (error as Error).message }, 500);
  }
});
...
app.delete('/jobs/:id', async (c) => {
  try {
    ...
    return c.json({ message: 'Error deleting job', error: (error as Error).message }, 500);
  }
});

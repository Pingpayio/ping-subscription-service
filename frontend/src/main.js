...
const setMessage = async (message, duration = 3000) => {
  messageElement.innerHTML = typeof message === 'string' ? message : message.outerHTML;
...
const formatNearAmount = (amount, decimals = 4) => {
  ...
  const formattedAmount = (Number(amount) / Math.pow(10, nearDecimals)).toFixed(decimals);
...
const registerWorker = async () => {
  try {
    await setMessage('<p class="text-lg font-bold mb-2">Registering Worker</p><p class="loading">Please wait</p>');
...
const renderActionButtons = () => {
  ...
  // Add event listeners
  document.getElementById('registerBtn').addEventListener('click', registerWorker);
  document.getElementById('refreshBtn').addEventListener('click', checkWorkerVerification);
};
...
const updateActionButtons = (isVerified) => {
  if (isVerified) {
    ...
    if (!document.getElementById('manageSubscriptionsBtn')) {
      ...
      manageBtn.className = 'bg-green-500 text-white px-4 py-2 rounded mt-4 block w-full';
      manageBtn.textContent = 'Manage Subscriptions';
      manageBtn.addEventListener('click', () => {
        setMessage('<p class="font-bold">Subscription Management</p><p>This feature is not implemented in this simple version.</p>');
      });
...
copyBtn.addEventListener('click', async () => {
  try {
    const accountId = accountIdElement.dataset.fullId || accountIdElement.textContent;
    if (navigator.clipboard && navigator.clipboard.writeText) {
      await navigator.clipboard.writeText(accountId);
      await setMessage('<p class="text-green-600 font-bold">Address copied to clipboard!</p>', 1000);
    } else {
      await setMessage('<p class="text-red-600 font-bold">Clipboard not supported</p>');
    }
  } catch (e) {
